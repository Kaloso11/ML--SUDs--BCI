# -*- coding: utf-8 -*-
"""
Created on Mon Oct  5 23:21:23 2020
...
df['AD'].replace (['Manageable','Addicted','Not addicted',"I don't know"],[0,0,0,1],inplace=True)
...
def _check_targets(y_test, y_pred):
    ...
    y_pred = model_logistic.decision_function(x_test)
    
    
    check_consistent_length(y_test, y_pred)
    type_true = type_of_target(y_test)
    ...
    if y_type in ["binary", "multiclass"]:
        ...
        if y_type == "binary":
            unique_values = np.union1d(y_test, y_pred)
            if len(unique_values) > 2:
                y_type = "multiclass"

    if y_type.startswith('multilabel'):
        y_test = csr_matrix(y_true)
        y_pred = csr_matrix(y_pred)
        y_test = 'multilabel-indicator'

    return y_type, y_test, y_pred

    logistic_fpr, logistic_tpr, threshold = roc_curve(x_test, y_pred)
...
# svm_fpr, svm_tpr, threshold = roc_curve(x_test, y_pred_svm)
