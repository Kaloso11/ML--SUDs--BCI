...
# # Define the CoherenceCustomBand function with Numba JIT
# # Define the CoherenceCustomBand function with Numba JIT
# #@jit(nopython=True)
# def CoherenceCustomBand_numba(eegData, i, j, fs, low_freq, high_freq):
#     nfft = eegData.shape[1]
#     f, Cxy = coherence(eegData[i, :, :], eegData[j, :, :], fs=fs, nfft=nfft, axis=0)
#     freq_mask = (f >= low_freq) & (f <= high_freq)
#     Cxy_masked = Cxy[freq_mask]
#     out = np.mean(Cxy_masked)
#     return out

# # Define the Coherence function using Numba
# def Coherence(eegData, fs=128):
#     n_channels = eegData.shape[0]
#     n_samples = eegData.shape[1]
#     n_epochs = eegData.shape[2]
#     bands = {'delta': (0.5, 4), 'theta': (4, 8), 'alpha': (8, 12), 'beta': (12, 25), 'gamma': (25, 55)}

#     coherence_results = {band: np.zeros((n_channels, n_channels, n_epochs)) for band in bands}

#     for i in range(n_channels):
#         for j in range(i, n_channels):
#             for epoch in range(n_epochs):
#                 for band, (low_freq, high_freq) in bands.items():
#                     coherence = CoherenceCustomBand_numba(eegData, i, j, fs, low_freq, high_freq)
#                     coherence_results[band][i, j, epoch] = coherence
#                     if i != j:
#                         coherence_results[band][j, i, epoch] = coherence

#     return coherence_results

# # Call the Coherence function
# # corr = Coherence(eegData)
...
def cross_correlation(eegData):
    n_channels = eegData.shape[0]
    n_epochs = eegData.shape[2]
    cross_correlation_results = np.zeros((n_channels, n_channels, n_epochs))
    ...
    for i in range(n_channels):
        for j in range(i, n_channels):
            for epoch in range(n_epochs):
                ccor = np.correlate(eegData[i, :, epoch], eegData[j, :, epoch], mode="full")
                absccor = np.abs(ccor)
                ccsi = (np.max(absccor) - np.mean(absccor)) / np.std(absccor)
                cross_correlation_results[i, j, epoch] = ccsi
                if i != j:
                    cross_correlation_results[j, i, epoch] = ccsi

    return cross_correlation_results

# Usage example:
# cross_correlation_matrix = calculate_cross_correlation(eegData)
...
def arma(eegData, orders=(2, 1, 2)):
    ...
    for chan in range(n_channels):
        for epoch in range(n_epochs):
            arma_mod = sm.tsa.ARIMA(eegData[chan, :, epoch], order=(p, d, q))
...
def CoherenceTheta(eegData, i, j, fs=128):
    nfft=eegData.shape[1]
    f, Cxy = signal.coherence(eegData[i,:,:], eegData[j,:,:], fs=fs, nfft=nfft, axis=0)#, window=np.hanning(nfft))
    out = np.mean(Cxy[np.all([f >= 4, f<=8], axis=0)], axis=0)
    return out

def CoherenceAlpha(eegData, i, j, fs=128):
    nfft=eegData.shape[1]
    f, Cxy = signal.coherence(eegData[i,:,:], eegData[j,:,:], fs=fs, nfft=nfft, axis=0)#, window=np.hanning(nfft))
    out = np.mean(Cxy[np.all([f >= 8, f<=12], axis=0)], axis=0)
    return out

def CoherenceBeta(eegData, i, j, fs=128):
    nfft=eegData.shape[1]
    f, Cxy = signal.coherence(eegData[i,:,:], eegData[j,:,:], fs=fs, nfft=nfft, axis=0)#, window=np.hanning(nfft))
    out = np.mean(Cxy[np.all([f >= 12, f<=25], axis=0)], axis=0)
    return out

def CoherenceGamma(eegData, i, j, fs=128):
    nfft=eegData.shape[1]
    f, Cxy = signal.coherence(eegData[i,:,:], eegData[j,:,:], fs=fs, nfft=nfft, axis=0)#, window=np.hanning(nfft))
    out = np.mean(Cxy[np.all([f >= 25, f<=55], axis=0)], axis=0)
    return out

##########
# Coherence - δ
def coherenceDeltaAll(eegData,fs):
	coh_res = []
